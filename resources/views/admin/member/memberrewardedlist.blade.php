...
<div class="page-wrapper">
			...
			<div class="page-content">
				...
				<div class="main-container">
					...
					<div class="page-header">
						...
						<!-- Breadcrumb start -->
						<ol class="breadcrumb">
							<li class="breadcrumb-item">Member Rewarded List</li>
						</ol>
						<!-- Breadcrumb end -->
 ...
 ?>
 <?php $randomsubs=membercontroller::memberrewarededdetails(); $i=1;  ?>
   @foreach($randomsubs as $rndmsubs)
   <tr>
											  <td>{{$i}}</td>
											   <td>{{$rndmsubs->member_id}}</td>
											   <?php $randomsubss=membercontroller::rewarededdetails($rndmsubs->reward_id); ?>
											    @foreach($randomsubss as $rndmsubsss)
											   <td>{{$rndmsubsss->Reward_Name}}</td>
											      <td>{{$rndmsubsss->Reward_Amount}}</td>
											   @endforeach
											
											
											  
											     <td>{{date('d-m-Y',strtotime($rndmsubs->Alloted_Date))}}</td>
											     
											     <td>
                                                <?php if ($rndmsubs->status=="Pending") {  ?>

                                               <a href="{{url('/admin/dispatchreward/'.$rndmsubs->id)}}" type="button" class="btn btn-primary" title="Dispatch"><i class="fa fa-check"></i></a>
											  <?php
