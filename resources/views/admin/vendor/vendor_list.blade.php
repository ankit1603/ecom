							...
							<div class="table-container">
								<div class="table-responsive">
									<table id="copy-print-csv" class="table custom-table">
										...
										<tbody>
                                            @foreach($vendorist as $vlst)

											<tr>
											  <td>{{$vlst->id}}</td>
											  <td>{{$vlst->name}}</td>
                                              <td>Contact:-{{$vlst->legal_name}}<br>
                                                email :-{{$vlst->email }}<br>
                                                current_billing_plan:-{{$vlst->current_billing_plan}}<br>
                                                Contact:-{{$vlst->legal_name}}<br>
                                                Address:-{{$vlst->address}}<br>
                                                </td>
											  <td>{{$vlst->created_at->format('d-m-yy')}}</td>
											  <td>{{$vlst->shopname}}</td>
											  <td>{{$vlst->status}}
											</td>
											  <td> <form action="{{url('admin')}}" method="POST"> {{csrf_field()}}
                                    <input type="hidden" name="username" value="{{$vlst->email}}" />
                                    <input type="hidden" name="password" value="{{$vlst->password}}" />
