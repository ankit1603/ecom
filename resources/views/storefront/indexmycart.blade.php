                         ...
                         <?php  if($user = Auth::user())
{
 ...
 <?php $total += $rndmsub->price * $rndmsub->quantity ; ?>
                    <tr>
                        <td>
                            <a href="#"><img src="upload/product/{{ $rndmsub->image }}" alt=""></a>
                        </td>
                        <td><a href="#">{{ $rndmsub->name }}</a>
                            <div class="mobile-cart-content row">
                                <div class="col-xs-3">
                                    <div class="qty-box">
                                        <div class="input-group">
                                            <input type="hidden" class="dataids" value="{{ $rndmsub->id }}" name="">
                                            <input type="number" name="quantity" class="form-control input-number quantitys" value="{{$rndmsub->quantity }}">
                                        </div>
                                    </div>
                                </div>
                                <div class="col-xs-3">
                                    <h2 class="td-color">{{ round($rndmsub->price) }}</h2></div>
                                <div class="col-xs-3">
                                    <h2 class="td-color"><a href="{{url('/removes/'.$rndmsub->id)}}" class="icon btn btn-danger btn-sm"><i class="ti-close"></i></a> <button class="btn btn-warning btn-sm update-carts" data-id="{{ $rndmsub->id }}"><i class="fa fa-refresh"></i></button></h2></div>
                            </div>
                        </td>
                        <td>
                            
                            <h2><i class="fa fa-rupee"></i> {{ round($rndmsub->price) }}</h2></td>
                        <td>
                            
                            <div class="qty-box">
                                <div class="input-group">
                                     <input type="hidden" class="dataid" value="{{ $rndmsub->id }}" name="">
                                    <input type="number"  class="form-control input-number quantity" onchange="" value="{{$rndmsub->quantity }}">
                                </div>
                            </div>
                        </td>
                        <td class="actions" data-th=""><a href="{{url('/removes/'.$rndmsub->id)}}" type="button" title="delete" class="icon "><i class="ti-close"></i></a><!--
                         <button class="btn btn-warning btn-sm update-cart" title="Refresh" data-id="{{ $rndmsub->id }}"><i class="fa fa-refresh"></i></button>--></td>
                        <td>
                            <h2 class="td-color"><i class="fa fa-rupee"></i> {{ round($rndmsub->price)*$rndmsub->quantity }}</h2></td>
                    </tr>
                        @endforeach
        ...
        @if(session('cart'))
            @foreach(session('cart') as $id => $details)
                <?php $total += $details['price'] * $details['quantity'] ?>
                 <tr>
                        <td>
                            <a href="#"><img src="upload/product/{{ $details['photo'] }}" alt=""></a>
                        </td>
                        <td><a href="#">{{ $details['name'] }}</a>
                            <div class="mobile-cart-content row">
                                <div class="col-xs-3">
                                    <div class="qty-box">
                                        <div class="input-group">
                                               <input type="hidden" class="dataids" value="{{ $id }}" name="">
                                            <input type="number" name="quantity" class="form-control input-number quantitys" value="{{ $details['quantity'] }}">
                                        </div>
                                    </div>
                                </div>
                                <div class="col-xs-3">
                                    <h2 class="td-color"><i class="fa fa-rupee"></i>{{ round($details['price']) }}</h2></div>
                                <div class="col-xs-3">
                                    <h2 class="td-color"><a href="{{url('/remove/'.$id)}}" class="icon btn btn-danger btn-sm"><i class="ti-close"></i></a><button class="btn btn-warning btn-sm update-cartsss" data-id="{{ $id }}"><i class="fa fa-refresh"></i></button></h2></div>
                            </div>
                        </td>
                        <td>
                            <h2><i class="fa fa-rupee"></i>{{ round($details['price']) }}</h2></td>
                        <td>
                            <div class="qty-box">
                                <div class="input-group">
                                    <input type="hidden" class="dataid" value="{{ $id }}" name="">
                                    <input type="number" name="quantity" class="form-control input-number quantity" value="{{ $details['quantity'] }}">

                                </div>
                            </div>
                        </td>
                        <td>><a href="{{url('/remove/'.$id)}}"type="button" class=" D btn btn-danger"><i class="fa fa-trash" title="Delete"></i></a>
                          <button class="btn btn-warning btn-sm update-cartss" title="Refresh" data-id="{{ $id }}"><i class="fa fa-refresh"></i></button>-</td>
                        <td>
                            <h2 class="td-color"><i class="fa fa-rupee"></i>{{ $details['quantity']*round($details['price']) }}</h2></td>
                    </tr>
                     @endforeach
                  @endif 
              <?php  }  ?>
                ...
                <table class="table cart-table table-responsive-md">
                    ...
                    <tr>
                        <td>total price :</td>
                        <td>
                            <h4 class="td-color"><i class="fa fa-rupee"></i>{{$total}}</h4></td>
                    </tr>
                    </tfoot>
                </table>
            </div>
...
@include('storefront.layouts.script')
    ...
    <script type="text/javascript">
         $(".quantity").change(function (e) {
           ...
           var ghg=document.getElementById("dataid");
           e.preventDefault();
           var ele = $(this);
      
            $.ajax({
               url: '{{ url('update-carts') }}',
               method: "patch",
               data: {_token: '{{ csrf_token() }}', id: ele.parents("tr").find(".dataid").val(), quantity: ele.val()},
          ...
          $(".quantitys").change(function (e) {
           ...
           var ele = $(this);
            ...
            $.ajax({
               url: '{{ url('update-carts') }}',
               method: "patch",
               data: {_token: '{{ csrf_token() }}', id: ele.parents("tr").find(".dataids").val(), quantity: ele.val()},
               success: function (response) {
          ...
          $(".update-carts").click(function (e) {
           ...
           var ele = $(this);
            ...
            $.ajax({
               url: '{{ url('update-cart') }}',
               method: "patch",
               data: {_token: '{{ csrf_token() }}', id: ele.attr("data-id"), quantity: ele.parents("tr").find(".quantitys").val()},
               success: function (response) {
           ...
           $(".update-cartss").click(function (e) {
           ...
           var ele = $(this);
            ...
            $(".update-cartsss").click(function (e) {
